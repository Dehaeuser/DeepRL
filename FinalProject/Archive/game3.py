...
class Game(tf.keras.Model):
    ...
    def __init__(self, sender, receiver, guesser, main_loss, aux_loss, sender_entr_coeff, receiver_entr_coeff, batch_size, max_len, sender_all_input):
...
# apply gradients to Aux_RNN seperately from and after Game
    ...
    def call(self, input_concepts, sender_input, targets, receiver_input):
        ...
        guesser_output = self.guesser(prev_hidden)
        receiver_thoughts = tf.identity(tf.stop_gradient(last_hidden))
        ...
        aux_loss = self.aux_loss(receiver_thoughts, message, receiver_input, guesser_output, targets)
